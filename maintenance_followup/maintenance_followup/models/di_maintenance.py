...
class DiMaintenanceStage(models.Model):
    """ Model for case stages. This models the main stages of a Maintenance Request management flow. """

    _name = 'di.maintenance.stage'
...
class DiMaintenanceEquipmentCategory(models.Model):
    
    _name = 'di.maintenance.equipment.category'
    ...
    equipment_ids = fields.One2many('di.maintenance.equipment', 'category_id', string='Equipments', copy=False)
    equipment_count = fields.Integer(string="Equipment", compute='_compute_equipment_count')
    maintenance_ids = fields.One2many('di.maintenance.request', 'category_id', copy=False)
    ...
    def _compute_equipment_count(self):
        equipment_data = self.env['di.maintenance.equipment'].read_group([('category_id', 'in', self.ids)], ['category_id'], ['category_id'])
        mapped_data = dict([(m['category_id'][0], m['category_id_count']) for m in equipment_data])
        for category in self:
            category.equipment_count = mapped_data.get(category.id, 0)

    def _compute_maintenance_count(self):
        maintenance_data = self.env['di.maintenance.request'].read_group([('category_id', 'in', self.ids)], ['category_id'], ['category_id'])
    ...
    def _alias_get_creation_values(self):
        values = super(DiMaintenanceEquipmentCategory, self)._alias_get_creation_values()
        values['alias_model_id'] = self.env['ir.model']._get('di.maintenance.request').id
        if self.id:
...
class DiMaintenanceEquipment(models.Model):
    
    _name = 'di.maintenance.equipment'
    ...
    def _track_subtype(self, init_values):
        ...
        if 'owner_user_id' in init_values and self.owner_user_id:
            return self.env.ref('maintenance_followup.di_mt_mat_assign')
        return super(DiMaintenanceEquipment, self)._track_subtype(init_values)
    ...
    category_id = fields.Many2one('di.maintenance.equipment.category', string='Equipment Category',
    ...
    maintenance_ids = fields.One2many('di.maintenance.request', 'equipment_id')
    ...
    maintenance_team_id = fields.Many2one('di.maintenance.team', string='Maintenance Team', check_company=True)
    ...
    def _compute_next_maintenance(self):
        ...
        for equipment in equipments:
            next_maintenance_todo = self.env['di.maintenance.request'].search([
                ('equipment_id', '=', equipment.id),
                ('maintenance_type', '=', 'preventive'),
                ('stage_id.done', '!=', True),
                ('close_date', '=', False)], order="request_date asc", limit=1)
            last_maintenance_done = self.env['di.maintenance.request'].search([
                ('equipment_id', '=', equipment.id),
                ('maintenance_type', '=', 'preventive'),
                ('stage_id.done', '=', True),
    ...
    def create(self, vals):
        equipment = super(DiMaintenanceEquipment, self).create(vals)
    ...
    def write(self, vals):
        ...
        return super(DiMaintenanceEquipment, self).write(vals)

    @api.model
    def _read_group_category_ids(self, categories, domain, order):
        #Read group customization in order to display all the categories in the kanban view, even if they are empty.
        category_ids = categories._search([], order=order, access_rights_uid=SUPERUSER_ID)
        return categories.browse(category_ids)

    def _create_new_request(self, date):
        self.ensure_one()
        self.env['di.maintenance.request'].create({
    ...
    def _cron_generate_requests(self):
        ...
        for equipment in self.search([('period', '>', 0)]):
            next_requests = self.env['di.maintenance.request'].search([('stage_id.done', '=', False),
                                                    ('equipment_id', '=', equipment.id),
                                                    ('maintenance_type', '=', 'preventive'),
...
class DiMaintenanceRequest(models.Model):
    ...
    def _default_stage(self):
        return self.env['di.maintenance.stage'].search([], limit=1)

    def _creation_subtype(self):
        return self.env.ref('maintenance_followup.di_mt_req_created')

    def _track_subtype(self, init_values):
        self.ensure_one()
        if 'stage_id' in init_values:
            return self.env.ref('maintenance_followup.di_mt_req_status')
        return super(DiMaintenanceRequest, self)._track_subtype(init_values)

    def _get_default_team_id(self):
        MT = self.env['di.maintenance.team']
    ...
    category_id = fields.Many2one('di.maintenance.equipment.category', related='equipment_id.category_id', string='Category', store=True, readonly=True)
    equipment_id = fields.Many2one('di.maintenance.equipment', string='Equipment',
                                   ondelete='restrict', index=True, check_company=True)
    user_id = fields.Many2one('res.users', string='Technician', tracking=True)
    stage_id = fields.Many2one('di.maintenance.stage', string='Stage', ondelete='restrict', tracking=True,
    ...
    maintenance_team_id = fields.Many2one('di.maintenance.team', string='Team', required=True, default=_get_default_team_id, check_company=True)
    duration = fields.Float(help="Duration in hours.")
    amount_quotation = fields.Float(string="Amount Quotation", help="Amount Quotation")
    ...
    def reset_equipment_request(self):
        ...
        first_stage_obj = self.env['di.maintenance.stage'].search([], order="sequence asc", limit=1)
    ...
    def create(self, vals):
        ...
        request = super(DiMaintenanceRequest, self).create(vals)
    ...
    def write(self, vals):
        ...
        res = super(DiMaintenanceRequest, self).write(vals)
        if vals.get('owner_user_id') or vals.get('user_id'):
            self._add_followers()
        if 'stage_id' in vals:
            self.filtered(lambda m: m.stage_id.done).write({'close_date': fields.Date.today()})
            self.activity_feedback(['maintenance_followup.di_mail_act_maintenance_request'])
        if vals.get('user_id') or vals.get('schedule_date'):
            self.activity_update()
        if vals.get('equipment_id'):
            # need to change description of activity also so unlink old and create new activity
            self.activity_unlink(['maintenance_followup.di_mail_act_maintenance_request'])
            self.activity_update()
        return res
    ...
    def activity_update(self):
        ...
        self.filtered(lambda request: not request.schedule_date).activity_unlink(['maintenance_followup.di_mail_act_maintenance_request'])
        for request in self.filtered(lambda request: request.schedule_date):
            ...
            updated = request.activity_reschedule(
                ['maintenance_followup.di_mail_act_maintenance_request'],
...
class DiMaintenanceTeam(models.Model):
    
    _name = 'di.maintenance.team'
    ...
    member_ids = fields.Many2many(
        'res.users', 'di_maintenance_team_users_rel', string="Team Members",
        domain="[('company_ids', 'in', company_id)]")
    color = fields.Integer("Color Index", default=0)
    request_ids = fields.One2many('di.maintenance.request', 'maintenance_team_id', copy=False)
    equipment_ids = fields.One2many('di.maintenance.equipment', 'maintenance_team_id', copy=False)

    # For the dashboard only
    todo_request_ids = fields.One2many('di.maintenance.request', string="Requests", copy=False, compute='_compute_todo_requests')
