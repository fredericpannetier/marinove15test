...
class DiTestEquipment(TransactionCase):
    """ Test used to check that when doing equipment/maintenance_request/equipment_category creation."""

    def setUp(self):
        super(DiTestEquipment, self).setUp()
        self.equipment = self.env['di.maintenance.equipment']
        self.maintenance_request = self.env['di.maintenance.request']
        self.res_users = self.env['res.users']
        self.maintenance_team = self.env['di.maintenance.team']
        self.main_company = self.env.ref('base.main_company')
        res_user = self.env.ref('base.group_user')
        res_manager = self.env.ref('maintenance_followup.di_group_equipment_manager')
        ...
        self.equipment_monitor = self.env['di.maintenance.equipment.category'].create({
            'name': 'Monitors - Test',
        })
    ...
    def test_10_equipment_request_category(self):
        ...
        maintenance_request_01 = self.maintenance_request.with_user(self.user).create({
            ...
            'color': 7,
            'stage_id': self.ref('maintenance_follow_up.di_stage_1'),
            'maintenance_team_id': self.ref('maintenance_followup.di_equipment_team_maintenance')
        })
        ...
        self.assertEqual(maintenance_request_01.stage_id.id, self.ref('maintenance_followup.di_stage_2'))

        # I check that change the maintenance_request stage on click statusbar
        maintenance_request_01.with_user(self.user).write({'stage_id': self.ref('maintenance_followup.stage_2')})
    ...
    def test_20_cron(self):
        ...
        maintenance_request_cron = self.maintenance_request.create({
            ...
            'color': 7,
            'stage_id': self.ref('maintenance_followup.di_stage_1'),
            'maintenance_team_id': self.ref('maintenance_followup.di_equipment_team_maintenance')
        })

        self.env['di_maintenance.equipment']._cron_generate_requests()
    ...
    def test_21_cron(self):
        ...
        self.env['di.maintenance.equipment']._cron_generate_requests()
