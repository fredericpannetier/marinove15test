...
class DiTestEquipmentMulticompany(TransactionCase):
    ...
    def test_00_equipment_multicompany_user(self):
        ...
        # Use full models
        Equipment = self.env['di.maintenance.equipment']
        MaintenanceRequest = self.env['di.maintenance.request']
        Category = self.env['di.maintenance.equipment.category']
        ResUsers = self.env['res.users']
        ResCompany = self.env['res.company']
        MaintenanceTeam = self.env['di.maintenance.team']

        # Use full reference.
        group_user = self.env.ref('base.group_user')
        group_manager = self.env.ref('maintenance_followup.di_group_equipment_manager')
        ...
        with self.assertRaises(AccessError):
            self.env['di.maintenance.stage'].with_user(user).create({
